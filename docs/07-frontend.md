...
│   ├── SearchBox.tsx        # 「検索」と「近い順」を 1 つのパネルで切り替える
│   ├── LibraryResultList.tsx # 候補リスト。**検索と近い順で共通**（2 つ目が出たので切り出した）
...
——この規則が初めて発動したのが `LibraryResultList.tsx`（検索結果と「近い順」で 2 つ目になった）。
ただし**両方とも地図画面の中でしか使わない**ので `components/` ではなく `map/` に置いている。
...
- 拒否された / 失敗した場合も**すべての機能が動く**設計にする。現在地が無いと使えないのは `nearby` だけ。
  当初は「ボタンを無効化して案内文を出す」と書いていたが、実装では**ボタンは押せるままにして
  パネル側に案内文を出す**ことにした（取得中だけ無効化）。押せないボタンは理由が伝わらず、
  「押したら許可を求められる」ほうが導線として素直だったため。
...
| 論点 | 決めごと |
|---|---|
| 検索範囲 | **`bbox` を送らない = 都全域。** 付けると八王子を見ながら「新宿」を検索して 0 件、という説明のつかない挙動になる |
| 地図のピン | **絞り込まない。** 検索は「探して飛ぶ」導線で、表示中のピンを減らすフィルタではない（それは喫煙区分の役割） |
| 喫煙区分フィルタ | 検索にも掛ける。地図と結果で食い違わせない |
| 上限 | 20 件。切れたら `truncated` を見て「上限 20 件を表示」と出す（一覧の `truncated` と同じ考え方） |
| 結果をクリック | `map.panTo` + `setZoom(16)` して詳細パネルを開く |
| 入力 | 250ms デバウンス。**日本語入力は変換の途中でも `onChange` が飛ぶ**（「し」「しん」「しんじゅく」…）ので、無いと 1 打鍵ごとにリクエストが出る |
| 結果パネルの位置 | 喫煙区分フィルタを**押し下げずに重ねる。** 375px で押し下げると地図がほとんど見えなくなる |

**カメラを制御にしない。** `panTo` を使うのは、`center` / `zoom` を渡す制御コンポーネントに
すると操作のたびに React へ戻ってきて地図が重くなるため（上の「地図本体」の表）。
現在地ボタンが元々やっていたのと同じ手。

> **`useMap()` は `<Map>` の内側でしか取れない。**
> 検索ボックスは地図の**外**（`overlayLeft`）に置きたいので、`<Map>` の中に
> 「`useMap()` の結果を親へ渡すだけ」のコンポーネント（`MapInstanceReporter`）を
> 噛ませてインスタンスを 1 段上げている。
>
> `<MapControl>` で検索ボックスごと地図の内側に入れるほうが短いが、
> **それだと地図が死んだときに検索ボックスまで消える。**
> `gm_authFailure` を直接叩いて確認したところ、外に置いた構成では
> 地図がプレースホルダに差し替わっても**検索と詳細パネルは動いたまま**だった
> （`panTo` だけが効かなくなる。`map` が `null` になるため）。
> 「地図が壊れてもアプリは動き続ける」は Day 3 で決めた方針。

⚠ **住所は 490 件中 264 件しか入っていない**（OSM 由来。`04-data-model.md`）。
「名称・住所で検索」と謳っているが、住所側のヒット率はデータの埋まり具合に依存する。

### 現在地から近い順

**検索と同じパネルを 2 モードで共有している**（`mode: 'search' | 'nearby' | null`）。
別々のパネルにしなかったのは、375px で左側に 3 段（検索・近い順・喫煙区分）積むと
**地図がほとんど見えなくなる**ため。

| 論点 | 決めごと |
|---|---|
| 権限を求めるタイミング | **「現在地から近い順」ボタン自身が求める。** 起動と同時には求めない（地図が見えている状態で押させるほうが許可率が高い） |
| 現在地が無いとき | **API を呼ばない。** `lat` / `lng` 無しで叩くと 400 になる。`useNearby` の `enabled` で保証し、パネルには拒否・取得失敗の案内文を出す |
| `queryKey` の座標 | 小数第 4 位（≒ 11m）に丸める。GPS の揺れでキーが変わってキャッシュが効かなくなるのを防ぐ（bbox の丸めと同じ発想） |
| 半径・件数 | 3km / 20 件（API の上限は 20km / 50 件） |
| 距離の表示 | 1km 未満はメートル、以上は小数 1 桁の km |

> **`useGeolocation` は `MapPage` で 1 回だけ呼ぶ。**
> 現在地ボタンは地図の**内側**（`MapControl`）、「近い順」は地図の**外側**にあり、
> それぞれがフックを呼ぶと**許可を 2 回求める**ことになる。`MapPage` が状態を持ち、
> `Geolocation` 型で両方に配る。
>
> 副作用として、「近い順」を押すと現在地ボタン側の効果も走って**地図が現在地へ寄る**。
> 近くの館を探しているときに自分の周りが見えるのは望ましいので、そのままにしている。
...
- API レスポンスの型は `types/api.ts` に**手書きする。** OpenAPI から `openapi-typescript` で生成する道もあるが、**そのための `drf-spectacular` を入れない判断をした**（`05-api.md`）。この規模なら手書きのほうが速い。
