...
8. ヘッダーの「登録」→ メールとパスワードで登録 → ログイン状態になる
9. **F5 でリロード → ログイン状態のまま**
10. ログアウト → Google ボタンでログイン → ログイン状態になる
11. お気に入りを 1 件登録 → `/favorites` に出る
12. `/login` を**アドレスバーに直接打って開く** → 404 にならない
