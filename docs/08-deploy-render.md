...
> **料金・無料枠の条件は変わりやすい。** 本ドキュメントの記述は作成時点の理解であり、**着手前に必ず [Render の料金ページ](https://render.com/pricing) と各サービスのドキュメントで最新の条件を確認すること。** 特に無料 PostgreSQL の有効期限は、この構成の前提そのものに関わる。
...
| `django-prac-db` | **PostgreSQL** | | **有効期限あり（要確認）。期限を過ぎると削除される** |
...
**2. 無料 PostgreSQL の有効期限**

Render の無料 Postgres は**作成から一定期間で失効し、インスタンスごと削除される**（期間は要確認）。
...
- 期限が来たら、新しい Postgres を作って `DATABASE_URL` を差し替える手順を `10-roadmap.md` の運用メモに書いておく
